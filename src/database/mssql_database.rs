...
use sqlx::mssql::{MssqlPool, MssqlRow};
use sqlx::{Column, Row, TypeInfo};
use chrono::{NaiveDate, NaiveDateTime};

use crate::dto::metadata_mapping::{DynamicStruct, PropertyType};
use crate::services::query::common::alias_manager::QueryAliasManager;
use crate::services::query::database::mssql::mssql_query::MssqlQuery;
use crate::dto::query_params::QueryParams;
use crate::dto::metadata::{ColumnMetadata, EntityMetadata};
use crate::database::Database;

pub struct MssqlDatabase(pub MssqlPool);
...
impl Database for MssqlDatabase {
    async fn execute<T: EntityMetadata>(&self, options: &QueryParams) -> Result<Vec<Value>, String> {   
        ...
        MssqlQuery.validate_query::<T>(options)?;

        let query = MssqlQuery.build_query::<T>(options, &mut query_alias_manger)?;
...
impl MssqlDatabase {
    fn parse_properties_hierarchy<T: EntityMetadata>(
        ...
        let metadata: &crate::dto::metadata::EntityDescription = T::metadata();
    
        let alias_map = query_alias_manager.get_field_alias();
    
        let column_names: Vec<String> = rows.first().unwrap().columns().iter()
            .map(|column| {
                alias_map.iter()
                ...
                for (i, part) in parts.iter().enumerate() {
                    if i == parts.len() - 1 {
                        ...
                        if let Some(relation) = current_metadata.relationships.get(*part) {
                            current_metadata = &relation.related_entity_metadata;
                        } else {
    ...
    fn fetch_column_value(row: &MssqlRow, column: &ColumnMetadata) -> Option<Value> {
