...
use super::metadata::{ColumnMetadata, EntityDescription};
        ...
        for (i, part) in parts.iter().enumerate() {
            if i == parts.len() - 1 {
                ...
                current_fields = match current_fields.entry(part.to_string()) {
                    std::collections::hash_map::Entry::Vacant(entry) => {
                        let new_property = PropertyType::new_relational(EntityDescription {
...
impl PropertyType {
    pub fn new_relational(metadata: EntityDescription, key: String) -> Self {
    ...
    pub fn new_property(metadata: ColumnMetadata, key: String) -> Self {
